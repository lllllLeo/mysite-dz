...
<%-- 							<td>${count-status.index }</td> --%>
							...
							<td style="text-align:left; padding-left: ${board.depth * 20 }px">
								...
								<a href="${pageContext.request.contextPath }/board/view/${board.no}">${board.title }</a>
							</td>
							<td>${board.name }</td>
							<td>${board.hit }</td>
							<td>${board.reg_date }</td>
							<c:if test="${authUser.no eq board.user_no }" >
								<td><a href="${pageContext.request.contextPath }/board/delete/${board.no }" class="del">삭제</a></td>
							</c:if>
						</tr>
					</c:forEach>
				</table>
				<div class="pager">
					<ul>
						<c:choose>
							...
							<c:otherwise>
								<li><a href="${pageContext.request.contextPath }/board/${map.firstPageNo - 10}">◀</a></li>
							</c:otherwise>
